...
const CreateBlock = ({handleGenerate, downloading, isBusy, saveCurrentBoard}) => {
    return (
        <div>
            ...
            <button onClick={downloading} disabled={isBusy} className={` ${isBusy ? "bg-gray-400 cursor-not-allowed" : "bg-black"} flex justify-center items-center text-center rounded-md cursor-pointer pulse px-3 w-full py-1 font-semibold text-white text-lg`} >
                {isBusy && (
            ...
            <button
                onClick={() => {
                    const name = prompt("Nombre para este cartón:", "Mi cartón");
